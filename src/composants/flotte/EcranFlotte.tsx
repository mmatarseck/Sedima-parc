...
function Interieur({ lignes }: { lignes: LigneFlotte[] }) {
  ...
  /* Le sous-titre compte ce que « Tous » montre : les sortis du parc et les
     archivés s'y retrouvent par leur filtre, pas dans le total de la flotte. */
  const auParc = toutes.filter((l) => !horsParc(l.vehicule));
  const archives = toutes.filter((l) => estArchive(l.vehicule)).length;
  const sortis = toutes.length - auParc.length - archives;
  ...
  return (
    <div className="flex flex-col gap-5 px-8 py-7 lg:h-full">
      <TitreEcran
        titre="Flotte"
        sousTitre={`${auParc.length} véhicules au parc${creees.length ? ` dont ${creees.length} créé${creees.length > 1 ? "s" : ""} ici` : ""}${sortis ? ` · ${sortis} sorti${sortis > 1 ? "s" : ""} du parc` : ""}${archives ? ` · ${archives} archivé${archives > 1 ? "s" : ""}` : ""}`}
        actions={
          <>
