...
import {
  VisionneusePiece,
  estImage,
} from "@/composants/interface/VisionneusePiece";
import type { PieceDossier } from "@/domaine/fiche";
import { date as formaterDate } from "@/lib/format";

/* ============================================================================
 * Un dossier de pièces : des familles, et la pièce ouverte à droite au clic.
 *
 ...
 * signe qu'à l'ouverture. Comme sur les dépenses (métier, 21 septembre
 * 2026), rien ne s'ouvre d'office : les familles ont toute la largeur, et
 * c'est au clic d'une pièce que la liste se replie et que la pièce s'affiche.
 * ==========================================================================*/
...
export function DossierPieces({
  ...
  familles: FamillePieces[];
  onRetirer?: (p: PieceDossier) => void;
  vide?: string;
}) {
  ...
  return (
    <div
      className={`grid grid-cols-1 items-start gap-5 ${choisie ? "xl:grid-cols-[minmax(0,340px)_minmax(0,1fr)]" : ""}`}
    >
      <div
        className={`order-last flex flex-col gap-4 xl:order-first ${choisie ? "" : "md:grid md:grid-cols-2 md:items-start"}`}
      >
        {familles.map((famille) => (
          <Carte
            ...
            action={
              famille.deposer ? (
                <button
                  type="button"
                  onClick={famille.deposer.onClick}
                  className="bouton-discret h-8 px-2 text-[12px]"
                  title={`${famille.deposer.libelle} — la pièce se joint sur la ligne qui la porte`}
                >
          ...
          >
            {famille.pieces.length === 0 ? (
              <p className="px-5 pb-4 text-[12.5px] text-texte-2">
                Aucune pièce
                {famille.deposer
                  ? " — « Déposer » ouvre la ligne qui la portera."
                  : "."}
              </p>
            ) : (
              <ul className="flex flex-col gap-1 px-3 pb-3">
                {famille.pieces.map((p) => {
                  const active =
                    choisie?.numero === p.numero &&
                    choisie.fichier === p.fichier;
                  return (
                    <li key={`${p.numero}-${p.fichier}`}>
                      <button
                        type="button"
                        onClick={() => setChoisie(active ? null : p)}
                        aria-current={active}
                      ...
                      >
                        {estImage(p.fichier) ? (
                          <IconeImage
                            className="mt-0.5 size-4 shrink-0 text-attenue"
                            strokeWidth={1.8}
                          />
                        ) : (
                          <FileText
                        ...
                        <span className="min-w-0">
                          ...
                          <span className="meta block truncate">
                            {[p.date ? formaterDate(p.date) : null, p.precision]
                              .filter(Boolean)
                              .join(" · ")}
                          </span>
                        </span>
      ...
      {choisie ? (
        <div className="min-w-0 xl:sticky xl:top-0">
          <VisionneusePiece
            fichier={choisie.fichier}
            libelle={choisie.libelle}
            precision={[
              choisie.date ? formaterDate(choisie.date) : null,
              choisie.precision,
            ]
              .filter(Boolean)
              .join(" · ")}
            ...
            actions={
              retirable ? (
                <button
                  type="button"
                  onClick={() => onRetirer(choisie)}
                  className="bouton-secondaire h-9 text-defavorable"
                  title="Retire le fichier de la ligne ; la ligne reste"
                >
