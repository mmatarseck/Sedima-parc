...
/* Le parc léger a rejoint « Flotte & véhicules » le 16 septembre 2026 : un
   seul parc, un seul référentiel, et la facette Régime pour y séparer
   l'exploitation du service et de la fonction. Les identifiants de ses
   rapports n'ont pas changé — favoris, réglages et liens les retrouvent. */
export type FamilleRapport = "flotte" | "couts" | "budget" | "carburant" | "maintenance" | "conformite" | "incidents" | "chauffeurs" | "achats" | "prestataires" | "transporteurs";

export const FAMILLE_RAPPORT: Record<FamilleRapport, { libelle: string; precision: string }> = {
  flotte: { libelle: "Flotte & véhicules", precision: "Le référentiel, les affectations, la disponibilité — et le parc léger : attributaires, plan car, forfaits carburant, renouvellement" },
  ...
  chauffeurs: { libelle: "Chauffeurs", precision: "Effectif, permis, activité et performance" },
...
/* Le régime d'usage — exploitation, service, fonction — est la facette qui
   sépare les camions du parc léger depuis que les deux partagent la famille. */
const REGIME = txt("regime", "Régime", { parDefaut: true, largeur: 120 });
...
const VEHICULE_SITUE = [IMMAT, VEHICULE, MARQUE, CATEGORIE, REGIME, CAT_FLOTTE, USAGE, ENERGIE_COL, BU, SITE];
...
export const RAPPORTS: DefinitionRapport[] = [
  /* ================= Flotte ================= */
  {
    ...
    description: "Le référentiel complet, une ligne par véhicule, tous régimes : identité, caractéristiques techniques, rattachement, statut, valeur et usure. Les sortis du parc et les archivés y sont, marqués — la facette « Au parc » les écarte.",
    famille: "flotte",
    unite: "véhicules",
    periode: false,
    ...
    colonnes: [
      ...VEHICULE_SITUE,
      eta("presence", "Au parc", { parDefaut: true, largeur: 130, precision: "présent, sorti, archivé" }),
      ...
      txt("chauffeur", "Titulaire ou attributaire", { parDefaut: true, largeur: 190, filtrable: false }),
  ...
  {
    id: "flotte-livraisons",
    libelle: "Livraisons",
    description: "Les livraisons de chaque véhicule, mois par mois : bons, jours de livraison, tonnage, clients, et la durée moyenne quand les heures sont saisies.",
    famille: "flotte",
    unite: "mois de livraison",
    periode: true,
    precisionPeriode: "la date du bon",
    lien: (l) => lienVehicule(l, "livraisons"),
    colonnes: [
      txt("mois", "Mois", { parDefaut: true, largeur: 130 }),
      ...VEHICULE_SITUE,
      num("bons", "Bons", { parDefaut: true, largeur: 100 }),
      num("jours", "Jours de livraison", { parDefaut: true, largeur: 165 }),
      vol("tonnage", "Tonnage", { parDefaut: true, largeur: 130, precision: "en tonnes, bons pesés" }),
      num("bonsSansPoids", "Bons sans poids", { largeur: 150 }),
      num("clients", "Clients", { parDefaut: true, largeur: 110 }),
      num("dureeMoyenne", "Durée moyenne", { largeur: 150, total: "moyenne", precision: "en minutes, livraisons dont les deux heures sont saisies" }),
      num("saisies", "Saisies dans l'application", { largeur: 200 }),
    ],
  },
  {
    id: "flotte-transferts",
    libelle: "Fiches de transfert",
    description: "Chaque remise d'un véhicule : qui remet, qui reçoit, le compteur, le carburant, les réserves et les signatures. La liste des fiches vit par véhicule ; ce rapport les réunit.",
    famille: "flotte",
    unite: "fiches",
    periode: true,
    precisionPeriode: "la date de la remise",
    lien: (l) => lienVehicule(l, "affectations"),
    colonnes: [
      txt("numero", "N°", { parDefaut: true, largeur: 140, filtrable: false }),
      dte("date", "Remise le", { parDefaut: true, largeur: 120 }),
      ...VEHICULE_SITUE,
      txt("motif", "Motif", { parDefaut: true, largeur: 190 }),
      txt("remettant", "Remis par", { parDefaut: true, largeur: 200 }),
      txt("recipiendaire", "Reçu par", { parDefaut: true, largeur: 200 }),
      dst("km", "Compteur", { largeur: 120, total: "aucun" }),
      num("carburant", "Carburant", { largeur: 115, total: "aucun", precision: "en pour cent du réservoir" }),
      num("reserves", "Réserves", { parDefaut: true, largeur: 110 }),
      num("equipementsManquants", "Équipements manquants", { largeur: 190 }),
      eta("statut", "Statut", { parDefaut: true, largeur: 190 }),
      txt("creePar", "Établie par", { largeur: 170 }),
    ],
  },
  {
  ...
  /* ---- Le parc léger, dans la même famille ----
     ...
     pèsent par BU, le renouvellement 2026, les immobilisés, le pool. Ils
     lisent la flotte à jour — un véhicule sorti ou archivé n'y est plus —,
     et les quatre rapports du haut couvrent aussi le parc léger, la facette
     Régime en main (16 septembre 2026). */
  {
  ...
  /* ================= Maintenance ================= */
  {
    ...
    colonnes: [
      ...
      txt("numero", "Réf.", { largeur: 150, filtrable: false }),
      txt("taches", "Tâches du catalogue", { parDefaut: true, largeur: 260, filtrable: false, precision: "affectées à l'intervention" }),
      oui("creee", "Saisie dans l'application"),
    ],
  },
  {
    id: "maintenance-ordres",
    libelle: "Services de maintenance",
    description: "Les services et où ils en sont : priorité, prestataire, tâches, main-d'œuvre, pièces, TTC, coût, immobilisation, pannes incluses — et l'intervention qui les clôt.",
    famille: "maintenance",
    unite: "ordres",
    periode: true,
    ...
    colonnes: [
      ...
      txt("objet", "Objet", { parDefaut: true, largeur: 280, filtrable: false }),
      txt("garage", "Garage", { parDefaut: true, largeur: 190 }),
      eta("priorite", "Priorité", { parDefaut: true, largeur: 130 }),
      txt("taches", "Tâches", { largeur: 280, filtrable: false }),
      mnt("mainOeuvre", "Main-d'œuvre", { largeur: 145 }),
      mnt("piecesAchetees", "Pièces achetées", { largeur: 155 }),
      mnt("magasin", "Pièces du magasin", { largeur: 160 }),
      mnt("totalHT", "Total HT", { largeur: 135 }),
      mnt("totalTTC", "Total TTC", { largeur: 135 }),
      mnt("brs", "BRS", { largeur: 120 }),
      mnt("cout", "Coût du service", { parDefaut: true, largeur: 155 }),
      mnt("montant", "Montant estimé", { largeur: 155 }),
      dur("immobilisation", "Immobilisation", { parDefaut: true, largeur: 150, precision: "en jours, du début à la fin des travaux" }),
      dur("immobilisationPrevue", "Immobilisation enregistrée", { largeur: 195, precision: "en jours" }),
      dte("dateFin", "Fin des travaux", { largeur: 140 }),
      num("pannes", "Pannes incluses", { largeur: 145 }),
      txt("numeroFacture", "N° de facture", { largeur: 150, filtrable: false }),
  ...
  },
  {
    ...
    libelle: "À faire",
    description: "Ce que les fiches réclament et qui n'est pas encore fait : observations de visite, préventif dû, suites d'incident, pannes signalées et services ouverts.",
    famille: "maintenance",
    unite: "travaux",
    periode: false,
    colonnes: [
      ...
      txt("ordreNumero", "Service déjà ouvert", { largeur: 165, filtrable: false }),
      txt("origineNumero", "Réf. d'origine", { largeur: 150, filtrable: false }),
    ],
  },
  {
    id: "maintenance-pannes",
    libelle: "Pannes signalées",
    description: "Les pannes et anomalies signalées : priorité, système, état, le service qui les répare et le délai de résolution. Les ouvertes toujours, les autres sur la période.",
    famille: "maintenance",
    unite: "pannes",
    periode: true,
    precisionPeriode: "la date du signalement",
    lien: (l) => lienVehicule(l, "maintenance"),
    colonnes: [
      txt("numero", "Réf.", { parDefaut: true, largeur: 150, filtrable: false }),
      dte("date", "Signalée le", { parDefaut: true, largeur: 125 }),
      ...VEHICULE_SITUE,
      eta("priorite", "Priorité", { parDefaut: true, largeur: 125 }),
      eta("etat", "État", { parDefaut: true, largeur: 140 }),
      txt("categorie", "Catégorie", { largeur: 170 }),
      txt("systeme", "Système", { parDefaut: true, largeur: 190 }),
      txt("description", "Le problème", { parDefaut: true, largeur: 280, filtrable: false }),
      dst("km", "Compteur", { largeur: 120, total: "aucun" }),
      txt("service", "Service", { parDefaut: true, largeur: 150, filtrable: false }),
      dte("resoluLe", "Résolue le", { largeur: 125 }),
      num("delai", "Délai de résolution", { parDefaut: true, largeur: 170, total: "moyenne", precision: "en jours" }),
      num("age", "Ouverte depuis", { largeur: 145, total: "moyenne", precision: "en jours" }),
      txt("declarant", "Déclarant", { largeur: 160 }),
    ],
  },
  {
    id: "maintenance-taches",
    libelle: "Tâches de maintenance",
    description: "Les tâches du catalogue comptées sur le parc : interventions et services clos, coût, véhicules touchés, part préventive — par tâche, système et catégorie.",
    famille: "maintenance",
    unite: "tâches",
    periode: true,
    precisionPeriode: "la date de l'intervention ou de la clôture du service",
    identifiant: "tache",
    colonnes: [
      txt("tache", "Tâche", { parDefaut: true, largeur: 300 }),
      txt("categorie", "Catégorie", { parDefaut: true, largeur: 170 }),
      txt("systeme", "Système", { parDefaut: true, largeur: 200 }),
      num("utilisations", "Utilisations", { parDefaut: true, largeur: 130 }),
      num("interventions", "Interventions", { largeur: 135 }),
      num("services", "Services clos", { largeur: 135 }),
      num("vehicules", "Véhicules", { parDefaut: true, largeur: 115, total: "aucun" }),
      num("preventif", "Préventif", { largeur: 115 }),
      num("curatif", "Curatif", { largeur: 115 }),
      mnt("cout", "Coût", { parDefaut: true, precision: "une intervention à plusieurs tâches se partage à parts égales" }),
      mnt("coutMoyen", "Coût moyen", { parDefaut: true, total: "moyenne" }),
      dte("derniere", "Dernière fois", { largeur: 135 }),
    ],
  },
  ...
  },
  {
    ...
    libelle: "Performance des chauffeurs",
    description: "Le classement du mois révolu : score, score par indicateur, rang, évolution et part de prime attribuée.",
    famille: "chauffeurs",
    unite: "chauffeurs",
    periode: false,
    identifiant: "nom",
    ...
    colonnes: [
      ...
      /* Le score de chaque indicateur ; la discipline, confidentielle, n'y figure pas. */
      num("kpiAcc", "Accidents", { parDefaut: true, largeur: 115, total: "moyenne", precision: "score sur 100" }),
      num("kpiInf", "Infractions", { parDefaut: true, largeur: 115, total: "moyenne", precision: "score sur 100" }),
      num("kpiInc", "Pannes et avaries", { parDefaut: true, largeur: 160, total: "moyenne", precision: "score sur 100" }),
      num("kpiCons", "Consommation", { parDefaut: true, largeur: 135, total: "moyenne", precision: "score sur 100" }),
      num("kpiPres", "Présence", { parDefaut: true, largeur: 110, total: "moyenne", precision: "score sur 100" }),
  ...
  {
    id: "conformite-pieces",
    libelle: "Pièces réglementaires attachées",
    description: "Un véhicule par ligne, et pour chaque pièce réglementaire — carte grise, assurance, salubrité, carte de transport, licence — si son scan est attaché et vaut encore, attaché mais échu, déclaré sans scan, ou absent.",
    famille: "conformite",
    unite: "véhicules",
    periode: false,
    identifiant: "immatriculation",
    lien: (l) => lienVehicule(l, "dossier"),
    colonnes: [
      IMMAT,
      VEHICULE,
      oui("complet", "Dossier complet", { parDefaut: true, largeur: 140, precision: "toutes les pièces requises attachées et valides" }),
      num("attachees", "Attachées", { parDefaut: true, largeur: 110, total: "aucun", precision: "valides, parmi les requises" }),
      num("attendues", "Requises", { parDefaut: true, largeur: 105, total: "aucun" }),
      num("manquantes", "Manquantes", { parDefaut: true, largeur: 120, total: "aucun" }),
      eta("carteGrise", "Carte grise", { parDefaut: true, largeur: 150 }),
      eta("assurance", "Assurance", { parDefaut: true, largeur: 150 }),
      eta("salubrite", "Certificat de salubrité", { parDefaut: true, largeur: 175 }),
      eta("carteTransport", "Carte de transport", { parDefaut: true, largeur: 160 }),
      eta("licence", "Licence de transport", { parDefaut: true, largeur: 165 }),
      CATEGORIE,
      REGIME,
      BU,
      SITE,
