-- ============================================================================
-- SEDIMA Parc — 0042 : « zéro panne, zéro accident » cesse de mentir.
--
-- Le tableau de bord affichait zéro panne, zéro accident et zéro chauffeur
-- indisponible cette semaine, et des courbes à zéro tout au long de 2026. Or le
-- registre des incidents n'a **aucune ligne** depuis la purge de la
-- démonstration, et celui des indisponibilités non plus : aucune source n'a été
-- chargée. Quarante-sept camions sans une panne en neuf mois, ce n'est pas un
-- parc exemplaire, c'est un registre qu'on ne tient pas encore.
--
-- C'est la règle de la carte grise (0037), appliquée aux registres : **un
-- registre sans aucune ligne n'est pas tenu**, et ses comptes ne sont pas des
-- zéros. La situation journalière dit maintenant si chacun porte au moins une
-- ligne ; les pastilles disent « — » et « aucun incident enregistré ». Dès le
-- premier incident déclaré, les zéros redeviennent des mesures.
--
-- On repart de 0041 : une fonction rejouée n'hérite de rien.
-- ============================================================================
...
as $$
  ...
  -- Le dernier voyage relevé, toutes dates confondues (0038) : même rôle que le
  -- dernier plein, pour la part confiée aux tiers.
  dernier_releve as materialized (select max(r.date) as jour from releve_transport r),
  -- Les registres tenus (0042) : un registre sans aucune ligne ne dit pas « aucune
  -- panne », il dit qu'on ne note pas encore les pannes.
  registres as materialized (select exists (select 1 from incident) as incidents, exists (select 1 from indisponibilite) as indisponibilites),
  ...
  statut_du_jour as (
    select
      ...
      coalesce(
        ...
        -- Sans trace ni réparation, on ne sait pas depuis quand (0041) : la date
        -- d'enregistrement de la fiche n'est pas celle de la panne.
        null::date
      ) as depuis_le
  ...
  faits as (
    select
      ...
      case when s.statut in ('en-service', 'en-backup') then 0 when s.depuis_le is null then null else greatest(1, s.jour - s.depuis_le + 1) end as immobilise_depuis_jours,
  ...
  flotte as (
    select
      ...
      (select dr.jour from dernier_releve dr) as dernier_releve,
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'flotte', jsonb_build_object(
      ...
      'registre_incidents', (select rg.incidents from registres rg),
      'registre_indisponibilites', (select rg.indisponibilites from registres rg),
      ...
      'dernier_releve_transport', case when vu.tiers then fl.dernier_releve end,
