-- ============================================================================
-- SEDIMA Parc — 0035 : le parc des prestataires au tableau de bord,
--                      et six champs rendus à la situation journalière.
--
-- DEUX CHOSES, et la première est une réparation.
--
-- 1. CE QUE 0031 AVAIT EFFACÉ SANS QUE PERSONNE LE VOIE.
--
-- 0031 corrigeait « prêt à charger » en réécrivant situation_journaliere().
-- Elle l'a réécrite à partir de la forme de 0016, pas de celle de 0018 — et
-- 0018 était deux choses à la fois : une réécriture en ensembles qui faisait
-- passer la fonction de 8,2 s à quelques dizaines de millisecondes, et la
-- migration qui alimentait les ordres de travail, la caisse et la cuve.
--
-- Résultat en production depuis le 10 septembre : six champs revenus à null —
...
-- antérieur qu'on croit connaître. Le banc `tester-tableau.mts` refuse
-- désormais qu'un champ de la flotte soit null quand le jeu de départ le
-- garnit — c'est ce contrôle qui manquait.
--
-- 2. LE PARC DES PRESTATAIRES (métier, 10 septembre 2026).
--
-- Le tableau de bord ne parlait que du parc propre. Huit champs s'ajoutent,
-- qui décrivent ce que les tiers transportent pour nous : leurs camions, les
-- mises à disposition du mois et leurs jours de panne, les affrètements non
-- livrés, le tonnage confié sur sept jours face au tonnage total, et les
-- factures reçues et non réglées.
--
-- Ils sont nuls — et non à zéro — pour qui ne lit pas le module Transporteurs.
-- Un zéro dirait « aucun affrètement » là où il faut dire « je ne sais pas » ;
-- la pastille affiche alors « — », comme la caisse avant d'avoir son module.
-- ============================================================================
...
as $$
  ...
  tiers_factures as materialized (
    select a.date_facture, a.date_reglement, coalesce(a.montant_facture, a.montant_convenu) as montant
      from affretement a where a.date_facture is not null and a.date_facture <= jusqua and a.statut <> 'annule'
    union all
    select m.date_facture, m.date_reglement, coalesce(m.montant_facture, ((m.jours_calendaires - m.jours_panne) * m.prix_jour)::bigint)
      from mise_a_disposition m where m.date_facture is not null and m.date_facture <= jusqua and m.statut <> 'annule'
    union all
    select p.date_facture, p.date_reglement, coalesce(p.montant_facture, (p.quantite * p.prix_unitaire)::bigint)
      from prestation p where p.date_facture is not null and p.date_facture <= jusqua and p.statut <> 'annule'
  ),
