...
import { KPI_CHAUFFEUR, classer, evaluer, scoresMensuels } from "../src/domaine/performance";
...
/* Refonte du 22 septembre 2026 : six indicateurs, score = moyenne des calculables, événements saisis. */
attendu(`six indicateurs au barème`, KPI_CHAUFFEUR.length === 6);
const premier = classes[0]!.evaluation;
const calcules = premier.kpis.filter((k) => k.score !== null);
attendu(`le score est la moyenne simple des indicateurs calculables (${calcules.length} sur 6)`, premier.score === Math.round(calcules.reduce((t, k) => t + k.score!, 0) / calcules.length));
const fichePremier = fiches.find((f) => f.ligne.id === premier.chauffeurId)!;
const sans = evaluer(fichePremier, "2026-08-01", "2026-08-31");
const avec = evaluer({ ...fichePremier, evenements: [{ numero: "EVC-TEST", date: "2026-08-10", nature: "disciplinaire", description: "essai", piece: null }] }, "2026-08-01", "2026-08-31");
const disc = (e: typeof avec) => e.kpis.find((k) => k.definition.code === "DISC")!;
attendu(`un cas disciplinaire pèse sur la discipline (${disc(sans).score} → ${disc(avec).score}) et sur le score (${sans.score} → ${avec.score})`, disc(avec).valeur === disc(sans).valeur! + 1 && (avec.score ?? 0) < (sans.score ?? 0));
const compense = evaluer({ ...fichePremier, evenements: [{ numero: "EVC-A", date: "2026-08-10", nature: "disciplinaire", description: "", piece: null }, { numero: "EVC-B", date: "2026-08-12", nature: "felicitation", description: "", piece: null }] }, "2026-08-01", "2026-08-31");
attendu(`une félicitation compense un événement négatif`, disc(compense).valeur === disc(sans).valeur);
const mensuels = scoresMensuels(fichePremier, ["2026-06", "2026-07", "2026-08", "2026-10"], "2026-09-02");
attendu(`l'évolution mensuelle : un score par mois passé, rien pour un mois à venir`, mensuels.slice(0, 3).every((m) => m.score !== null) && mensuels[3]!.score === null);
