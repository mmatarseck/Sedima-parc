...
attendu("puis une dépense par part de ligne, avec la facture jointe — sauf le magasin", ecritures.filter((e) => e.type === "depense").length === depenses.length && ecritures.filter((e) => e.type === "depense" && e.valeurs.origine === "caisse").length > 0 && ecritures.filter((e) => e.type === "depense" && e.valeurs.origine === "caisse").every((e) => e.valeurs.photo === service.pieces![0]) && ecritures.filter((e) => e.type === "depense" && e.valeurs.origine === "stock").every((e) => e.valeurs.photo === null && e.valeurs.beneficiaire === "Magasin SEDIMA"));
